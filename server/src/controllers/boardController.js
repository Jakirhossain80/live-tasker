...
const validateColumns = (columns) => {
    ...
    for (const column of columns) {
        ...
        if (boardColumn.title.trim().length > 60) {
            return "Board column title cannot be longer than 60 characters";
        }
        if (typeof boardColumn.order !== "number" ||
...
const validateCreateBoardBody = (body) => {
...
const validateUpdateBoardBody = (body) => {
...
const getBoardsByWorkspace = asyncHandler(async (req, res) => {
    const boards = await boardService.getBoardsByWorkspace(getRouteParam(req, "workspaceId"), getAuthUserId(req));
    res.json({
        success: true,
        data: {
            boards,
...
const getBoardById = asyncHandler(async (req, res) => {
    const board = await boardService.getBoardById(getRouteParam(req, "boardId"), getAuthUserId(req));
    res.json({
        success: true,
        data: {
            board,
...
const deleteBoard = asyncHandler(async (req, res) => {
    await boardService.deleteBoard(getRouteParam(req, "boardId"), getAuthUserId(req));
    res.json({
        success: true,
