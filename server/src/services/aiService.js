...
const getClient = () => {
    ...
    if (!apiKey) {
        throw createHttpError("Gemini API key is not configured", 500, "GEMINI_API_KEY_MISSING");
    }
    return new genai.GoogleGenAI({ apiKey });
};
const buildTaskDescriptionPrompt = (title) => {
    return [
...
const generateTaskDescription = async (title) => {
    ...
    if (!description) {
        throw createHttpError("Failed to generate task description", 502, "AI_DESCRIPTION_GENERATION_FAILED");
    }
    return description;
};
module.exports = {
