...
const validateCommonTaskFields = (body) => {
...
const validateMoveTaskBody = (body) => {
    if (!isStringWithValue(body.status)) {
        return "Task status is required";
    }
...
const getTasksByBoard = asyncHandler(async (req, res) => {
    const tasks = await taskService.getTasksByBoard(getRouteParam(req, "boardId"), getAuthUserId(req));
    res.json({
        success: true,
        data: {
            tasks,
...
const getTaskById = asyncHandler(async (req, res) => {
    const task = await taskService.getTaskById(getRouteParam(req, "taskId"), getAuthUserId(req));
    res.json({
        success: true,
        data: {
            task,
